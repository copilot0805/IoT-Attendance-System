...
const postAttendanceAPI = async (req, res) => {
    try {
        ...
        console.log(`Nhận được ảnh từ camera ESP32 (Kích thước: ${imageBuffer.length} bytes)`);

        // =====================================================================
        // BƯỚC 1: CHỈ NHỜ AI TRÍCH XUẤT 512 SỐ (KHÔNG CHO AI TỰ SO SÁNH NỮA)
        // =====================================================================
        const aiResponse = await axios.post('http://127.0.0.1:5000/extract', imageBuffer, {
        ...
        if (bestMatch && bestMatch.distance <= THRESHOLD) {
            ...
            // Tương lai bạn có thể viết lệnh INSERT INTO attendance_logs ở đây!
        ...
        if (error.response && error.response.data) {
            return res.status(400).json({ error: error.response.data.detail || 'Lỗi AI' });
        }
        return res.status(500).json({ error: 'Lỗi hệ thống máy chủ' });
    }
};
